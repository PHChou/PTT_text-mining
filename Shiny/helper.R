...
super_jieba_cloud <- function ( rawdata, filters1, filters2, col1,  
  ...
  d2 <- super_grepl( filters = filters2, rdata = rawdata, col= col1 )
  ...
  #刪除掉有stopword的資料，剩下的就是我要的(畫cloud用的)
  f.word.table <- stop_words_exclude( stopwords = stopword,
                                      rdata = word.table2,
                                      col = 1 )
  # 先選stop word
  if( output_select=="stopword slecet" ){  word.table2  } else 
    # 畫cloud
    if( output_select=='cloud' ){  library(wordcloud2)
      
      wordcloud2::wordcloud2(data = f.word.table[1:round(nrow(f.word.table)/10,0),] )
